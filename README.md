...
In the `examples-blog-posts-generated` folder, you can find examples of blog posts generated by the application. Each blog post is stored in a separate folder containing two files:

1. `info.txt`: Contains information about the video, such as duration, ID, and link.
2. `blog_post.txt`: Contains the blog post generated from the video.
